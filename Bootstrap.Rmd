...
### The Model  
We fit a simple logistic regression to predict the probability of **Good vs. Bad** credit qaulity. We initially include all independent variables, then uses stepwise variable selection to fit the final model.
...
plot(perf, col="navyblue", cex.main=1,
     main= paste("Logistic Regression ROC Curve: AUC =", round(auc,2)))
...
for(i in 1:R){
    ...
    pred = prediction(prob, data.boot$credit)
    auc = performance(pred,"auc")@y.values[[1]][1]
    B[i, 2] = auc
}
