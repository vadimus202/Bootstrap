...
The plots below illustrate the distribution of bootstrapped model coefficients and 95% empirical confidence intervals (thick black line at the bottom of the chart). For comparison, the graphs also show the normal distributions (red bell curves) and 95% confidence intervals (thick red line) that are implied by the coefficients and their standard errors (assuming normality). 
...
for(i in 2:nrow(coef.comp)){
  ...
  p <- ggplot(data.frame(coeff = coef.boot$t[,i]), aes(x=coeff)) +
    geom_histogram(aes(y=..density..), 
                   ...
                   fill='powderblue', color='black') +
    stat_function(fun = dnorm, color='red',
                  args = list(mean = coef.comp$coef[i], 
                              sd = coef.comp$se[i])) + 
    geom_line(aes(x, y), ci.df, size=2, color='red') +
...
- The red dashed lines mark 50%, 95%, and 99% confidence intervals.
...
ggplot(B, aes(x=studentYes, y=balance)) +
  geom_point(size=.7, color='navyblue')+
    stat_ellipse(type = "norm", linetype = 1, level = 0.001, color='red', size=2) +
    stat_ellipse(type = "norm", linetype = 2, level = 0.50, color='red', size=.8) +
  stat_ellipse(type = "norm", linetype = 2, level = 0.95, color='red', size=.8) +
  stat_ellipse(type = "norm", linetype = 2, level = 0.99, color='red', size=.8) +
